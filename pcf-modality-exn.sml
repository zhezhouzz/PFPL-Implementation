...
and layoutm (Ifz (v1, m1, x, m2)) = "(ifz " ^ (layoutv v1) ^ " then " ^ (layoutm m1) ^ " else " ^ x ^ "." ^ (layoutm m2) ^ ")"
  ...
  | layoutm (Raise v) = "(raise " ^ (layoutv v) ^ ")"
  | layoutm (Bnd (v1, x, m2, m3)) = "(" ^ x ^ "← " ^ (layoutv v1) ^ "; " ^ (layoutm m2) ^ "↑" ^ (layoutm m3) ^ ")"
...
and layoutm (Ifz (v1, m1, x, m2), ifp) =
  ...
  | layoutm (Raise v, ifp) =
    if ifp then
        "(raise " ^ (layoutv (v, true)) ^ ")"
    else
        "raise " ^ (layoutv (v, true))
  | layoutm (Bnd (v1, x, m2, m3), _) =
    if true then
        "(" ^ x ^ "← " ^ (layoutv (v1, false)) ^ "; " ^ (layoutm (m2, false)) ^ " ↗ " ^ (layoutm (m3, false)) ^ ")"
    else
        x ^ "← " ^ (layoutv (v1, false)) ^ "; " ^ (layoutm (m2, false)) ^ " ↗ " ^ (layoutm (m3, false))
...
and substm (m, id, vid) =
    case m of
      ...
      | Raise v => Raise (substv (v, id, vid))
      | Bnd (v1, x, m2, m3) =>
        if x = id
        then Bnd (substv (v1, id, vid), x, m2, m3)
        else Bnd (substv (v1, id, vid), x, substm (m2, id, vid), substm (m3, id, vid))
end
...
and staticm (gamma, m) =
    case m of
        ...
        (case (staticv (gamma, v1), staticm (gamma, m2),
           ...
           | (Strict N, AnyMty, Weak t3) => Weak t3
           | (Strict N, Weak t2, AnyMty) => Weak t2
           | (Strict N, AnyMty, AnyMty) => AnyMty
           | _ => NoMty
        )
      | Ap (v1, v2) =>
      ...
      | Raise v =>
        (case staticv (gamma, v) of
             Strict Ty.Exn => AnyMty
           | _ => NoMty)
      | Bnd (v1, x, m2, m3) =>
        (case (staticv (gamma, v1)) of
             Strict (Ty.Comp t1) =>
                 (case (staticm (update (gamma, x, t1), m2),
                       staticm (update (gamma, x, Ty.Exn), m3)) of
                     (Weak t2, Weak t3) =>
                     if Ty.eq (t2, t3) then Weak t3 else NoMty
                   | (Weak t2, AnyMty) => Weak t2
                   | _ => NoMty)
           | _ => NoMty
        )
fun evalm e =
             ...
             (case stepm (E.substm (m1, y, E.Comp (t, y, m1))) of
                ...
                | Done (E.Raise v1) =>
                  let
                      val v1 = E.substv (v1, x, v1)
                  in
                      Computation (E.substm (m3, x, v1))
                  end
                | Computation m1 => Computation (E.Bnd (E.Comp (t, y, m1), x, m2, m3))
...
let
    ...
    val exp3 = Bnd (Comp0 (NatComp, Ret (Comp0 (Nat, Ifz (Two, Ret Z, "x0", Ret x0)))), "x0", Bnd (x0, "x1", Ret x1, Raise x1), Raise x0)
    val _ = pLn (Static.evalm exp3)
    val exp4 = Bnd (Comp0 (NatComp, Ret (Comp0 (Nat, Ifz (Two, Ret Z, "x0", Raise Exn)))), "x0", Bnd (x0, "x1", Ret x1, Raise x1), Raise x0)
    val _ = pLn (Static.evalm exp4)
    val exp5 = Comp (Nat2Nat, "f0",
                     Ret (Lam (Nat, "x0",
                               Ifz (x0, Ret Z, "x1",
                                    Bnd (f0, "x2", Ap (x2, x1), Raise x2)
                                   )
                              )
                         )
                    )
    val exp5 = Bnd (exp5, "g", Ap (V "g", Three), Raise Exn)
    (* val exp6 = Bnd (Comp (Ty.Pf (Nat, Nat), "r", Ret(Lam (Nat, "y", Ifz (S(S(S Z)), Ret Z, "x", Ret (V "y"))))), "g", Ap (V "g", S(S(S Z)))) *)
    val exp7 = Comp (Nat2Nat, "f0",
                     Ret (Lam (Nat, "x0",
                               Ifz (x0, Raise Exn, "x1",
                                    Bnd (f0, "x2", Ap (x2, x1), Raise x2)
                                   )
    ...
    val exp7 = Bnd (exp7, "g", Ap (V "g", Three), Raise Exn)
